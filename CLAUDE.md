...
Angular 20.3.x portfolio, standalone components, **zoneless change detection** (`provideZonelessChangeDetection()` in [app.config.ts](src/app/app.config.ts)). Page-based structure, no NgModules. **Supabase backend** for dynamic content (see `supabase/README.md` for setup; schema + seed SQL live in `supabase/`). See [docs/PLAN.md](docs/PLAN.md) for current build status and outstanding items — this codebase is actively worked on by both Claude Code and Cursor, so check it (and `git log`/`git diff` against HEAD) before assuming a file's working-tree state reflects the latest intended version.
...
- **Admin** (`src/app/pages/admin/`): Supabase Auth (email+password, signups disabled — single user created in dashboard), functional `authGuard`, generic [admin-crud.component.ts](src/app/pages/admin/shared/admin-crud.component.ts) configured per table via route `data.config` from [crud-config.ts](src/app/pages/admin/shared/crud-config.ts). Admin components use inline templates + Tailwind only (no component SCSS — avoids the 4kB `anyComponentStyle` budget). All admin screens (Login, Layout, Messages, Profile Editor, generic CRUD, blog admin) share one design language defined globally in [styles.scss](src/styles.scss): `.card-panel`, `.admin-form`, `.admin-table`, `.admin-dialog`, `.form-input`/`.form-textarea`, `.btn-send`, `.field-error` — the same classes the public Contact form uses. **Reuse these classes for any new admin screen** rather than styling from scratch; there is no per-component styling story for admin beyond them.
...
- **PrimeNG v20** (Aura preset) bootstrapped once in [main.ts](src/main.ts) via `useTheme({ preset: Aura, options: { darkModeSelector: '.dark', ... } })` + `updatePrimaryPalette(palette('#10b981'))`, *before* `bootstrapApplication`. `darkModeSelector` must be a real CSS selector (`'.dark'`, not the bare word `'class'`) and the primary color must go through the `palette()` helper (not a raw hex string) — either mistake silently breaks dark-mode theming for every PrimeNG component, including portaled overlays (`p-select` panels, `p-confirmdialog`) that no amount of scoped CSS can reach. Import PrimeNG modules per-component from `primeng/*` (e.g. `ButtonModule`, `Ripple`).
- **Media uploads**: [media.service.ts](src/app/core/media.service.ts) wraps Supabase Storage for image/PDF uploads, used by the Profile Editor (avatar, resume) and the generic CRUD form (project images).
