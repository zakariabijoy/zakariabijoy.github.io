...
- **Pages** (`src/app/pages/`): `home`, `work`, `resume`, `contact`, plus a guarded `admin/` area — lazy-loaded in [app.routes.ts](src/app/app.routes.ts) (`loadComponent` for pages, `loadChildren` for admin). No separate "sections" components; content is embedded directly in each page template.
